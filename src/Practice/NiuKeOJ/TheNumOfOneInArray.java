...
ACMer希望你们帮帮他,并把问题更加普遍化,可以很快的求出任意非负整数区间中1出现的次数。
 ...
 不对，注意11算作两次
 */
public class TheNumOfOneInArray {
    ...
    /*
 ? 主要思路：设定整数点（如1、10、100等等）作为位置点i(对应n的各位、十位、百位等等),分别对每个数位上有多少包含1的点分析
    根据设定的整数位置，对n进行分割，分为两部分，高位n/i，低位n%i

    1. 当i表示百位，且百位对应的数>=2,如n=31456,i=100，则a=314,b=56，此时百位为1的次数有a/10+1=32（最高两位0~31），
    每一次都包含100个连续的点，即共有(a/10+1)*100=3200个点的百位为1
    2. 当i表示百位，且百位对应的数为1，如n=31156,i=100，则a=311,b=56，此时百位对应的就是1，则共有a/10(最高两位0-30)次是
    包含100个连续点，当最高两位为31（即a=311），本次只对应局部点00~56，共b+1次，
    ...
    */
    public static int solve2(int n){
        ...
        for(int m = 1; m<=n;m++){
            nCount += (n/m+8)/10*m +(n/m%10==1?n%m+1:0); //记得加括号
        }
        return nCount;
    }
